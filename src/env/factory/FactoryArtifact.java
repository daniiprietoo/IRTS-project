// ============================================================
//  FactoryArtifact.java  — package factory
//  CArtAgO 3.x artifact for the Assembly Factory simulation.
//
//  Compile:
//    javac -cp jason-3.3.jar:cartago-3.x.jar \
//          src/env/factory/FactoryArtifact.java -d bin
//
//  Design notes:
//   - All robot state is exposed as observable properties.
//     Any agent that focuses on this artifact perceives them
//     automatically as Jason beliefs.
//   - Bin states are individual obs. properties bin_1..bin_6.
//     Agent files include a binfull(N) rule that derives from
//     those properties so the rest of the codebase is unchanged.
//   - The GUI runs on its own daemon AnimationThread (30 fps).
//   - Operations are named identically to the original
//     executeAction cases so agent .asl files need minimal edits.
// ============================================================
package factory;
...
public class FactoryArtifact extends Artifact {
    ...
    private Logger log = Logger.getLogger(FactoryArtifact.class.getName());
    ...
    void init() {
        ...
        // Robot positions (updated after every operation)
        ...
        // Assembly area lock state (for GUI + agent rules)
        defineObsProperty("area_locked_1", false);
        defineObsProperty("area_locked_2", false);

        // Static constants (never change)
        defineObsProperty("bins",    BINS);
        defineObsProperty("parts",   PARTS);
        defineObsProperty("holders", HOLDERS);
        defineObsProperty("joints",  JOINTS);

        // Part / position constants
        for (int i = 0; i < PARTS; i++) {
            defineObsProperty("partLength", i+1, PART_LENGTHS[i]);
            defineObsProperty("holderPos",  i+1, HOLDER_POS[i][0], HOLDER_POS[i][1]);
            defineObsProperty("partPos",    i+1, PART_POS[i][0],
                                                  PART_POS[i][1], PART_POS[i][2]);
            defineObsProperty("binPos",     i+1, BIN_POS[i][0], BIN_POS[i][1]);
        }
        for (int i = 0; i < JOINTS; i++) {
            defineObsProperty("jointPos", i+1, JOINT_POS[i][0], JOINT_POS[i][1]);
        }
    ...
    // ─────────────────────────────────────────────────────────
    //  @OPERATION — one per action (replaces the old switch block)
    // ─────────────────────────────────────────────────────────

    @OPERATION
    void pick_part(String ag, int partnum) {
        model.pickPart(ag, partnum);
        updateObsProperty("bin_" + partnum, false);
        pushPositions();
    }
    ...
    // Push current robot positions to observable properties
    private void pushPositions() {
        updateObsProperty("gripper",
            model.gripperPosition[0],
            model.gripperPosition[1],
            model.gripperAngle);
        updateObsProperty("welder",
            model.welderPosition[0],
            model.welderPosition[1]);
        updateObsProperty("mover",
            model.moverPosition[0],
            model.moverPosition[1]);
    }
    ...
    class FactoryView {
        ...
        FactoryView(FactoryModel model) {
